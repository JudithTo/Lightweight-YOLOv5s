...
def modify_nms(boxes, scores, iou_thres, GIoU=False, DIoU=True, CIoU=False):#nms修改为ciounms
    # 按conf从大到小排序
    ...
    while B.numel() > 0:
        ...
        # 找到符合阈值的下标
        B = B[torch.nonzero(iou <= iou_thres).reshape(-1) + 1]
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #i = modify_nms(boxes, scores, iou_thres)  # diou-nms
...
def save_one_box(xyxy, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False):
    ...
    # print("左上角的坐标为:("+str(int(xyxy[0,0]))+"."+str(int(xyxy[0,1]))+").右下角的坐标为("+str(int(xyxy[0,2]))+"."+str(int(xyxy[0,3]))+")")
